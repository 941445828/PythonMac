...
def clickElement(element, useNativeEvents):
    if useNativeEvents > 0:
        ...
        actions = ActionChains(startWebDriverSession)
...
def openSystemPreferencesPane(driver, pane):
    ...
    if pane:
        ...
        ActionChains(driver).key_down(Keys.COMMAND).send_keys('a').key_up(Keys.COMMAND).send_keys(
            pane + Keys.RETURN).perform()
    else:
...
def closeWithoutSaving(driver):
    ...
    if closeButton:
        ...
        time.sleep(1)
        dontSaveButton = existsElementByXPath(driver,
